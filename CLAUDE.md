...
- **`client.py`** — `OmnisClient`, the async httpx-based client that talks to a Primo instance. All Primo API interaction lives here:
  ...
  - `get_fines()` — `GET /primaws/rest/priv/myaccount/fines`, returns `List[Fine]`. Accounts with no fines return `data: {}` (no `fines` key at all, not an empty list) — handled via `.get("fines", {}).get("fine", [])`. Fine amounts (`finesum`/`originalfinesum`) use a Polish-formatted string (`"0,20 PLN"`, comma decimal + trailing currency) that is a **different format** from the plain `"0.00"` used by `myaccount/counters` — `Fine.from_api()` parses it via `_parse_fine_amount()` into separate `amount: float`/`currency: str` fields.
  - `get_requests()` — `GET /primaws/rest/priv/myaccount/requests`, returns `List[RequestItem]` flattened from the response's six top-level categories (`holds`/`photocopies`/`bookings`/`cdls`/`ills`/`acqs`, each `{"<category>": [...]}`). `RequestItem` deliberately has no named fields beyond `category` — the per-item shape (e.g. what a single `hold` actually contains) has never been observed live, since no verification could find an account with an active hold/request; see `docs/plans/account-actions-api.md` before adding typed fields or any mutating action (cancel/place hold, pay fine) on top of this.
  ...
  - Pydantic models `Loan`, `Fine`, `RequestItem`, `BookDetails`, `UserInfo`, `SearchResult`, `BookVersion`, `BranchAvailability` define the data shapes; `Loan`/`Fine` use field aliases matching Primo's raw JSON keys (e.g. `loanid`, `duedate`, `fineid`) and their respective `from_api()` classmethods normalize derived fields (`Loan.renewable` from `renew`; `Fine.amount`/`currency` from `finesum`). The search models are built manually (not via aliases) since they merge data from multiple endpoints.
...
Each `delivery.holding` item also carries `subLocation` (a real street address, e.g. `"ul. Druskienicka 32"`) and `stackMapUrl` (a ready-made `maps.app.goo.gl` short link) — captured as `BranchAvailability.sub_location`/`maps_url`, shown in the CLI table via `--search --address`. Unlike `branches.py`'s scraper below, this comes straight from Primo/Alma and works for any tenant, at no extra API cost since it's already part of the `delivery` response fetched for availability.

`BookVersion` also carries `series` (from `addata.seriestitle`, e.g. `"O Mordimerze Madderdinie / Jacek Piekara ; 1"` — a compact "cycle title / author ; number" string, preferred over the noisier `$$Q`-duplicated `display.series`), plus `genres`/`subjects`/`language`/`physical_description` (from `display.genre`/`subject`/`language`/`format`). `series` is shown as a subtitle under the title in `display_search_results` unconditionally (cheap, and it's the one field that actually disambiguates same-title records — see the `frbrgroupid` split under "Catalog search" above); the rest only print (as a per-edition `Panel`) when `-v`/`--verbose` is passed together with `--search`.
...
- **`cli.py`** — `omnis-cli` entry point (`omnis.cli:main` in `pyproject.toml`). Responsibilities:
  ...
  - `--search QUERY` (optionally with `--branch NAME`, `--address`, and/or `-v`/`--verbose`) short-circuits `async_main()` right after `load_config()`, before the account-wizard/loans-fetch logic, and returns — it reuses the first configured account purely to obtain login credentials/tenant info, and has its own renderer (`display_search_results`) rather than touching the loan table/json/csv renderers. `--address` adds an "Address" column (street address + maps link, from `BranchAvailability.sub_location`/`maps_url`) — no extra request, since that data is already part of the `delivery` response. `--verbose` (the same global flag that adds loan-date/renewability columns to the plain loan table) instead prints a details `Panel` per edition (ISBN, language, physical description, genre, subject) when combined with `--search`.
  ...
  - `--fines`/`--requests` each short-circuit right after `load_config()` (same pattern as `--search`), but unlike `--search` they fetch from *every* configured account concurrently (`fetch_account_fines`/`fetch_account_requests` + `asyncio.gather`), since fines/holds are inherently per-account data rather than shared catalog data. Each has its own table/json/csv renderers; `--requests` table output shows raw JSON per row instead of named columns, reflecting that `RequestItem`'s per-item shape is unverified (see `client.py` notes above).
  - `_redact_account()` strips the plaintext `password` key from an account dict before it's placed into any result destined for `--format json` (`PydanticEncoder` serializes the whole `results` list, including `res["account"]`) — used by `fetch_account_data`, `fetch_account_fines`, `fetch_account_requests`. Without it, `--format json` leaks the account's plaintext password into stdout/piped output; nothing downstream reads `account["password"]` after login, so it's always safe to strip.
